...
export function FileUpload({ onUploadComplete }: FileUploadProps) {
  ...
  return (
    <div
      {...getRootProps()}
      className={`w-full rounded-xl border-2 border-dashed py-10 px-6 text-center transition sm:py-12 sm:px-8 ${
        uploadState === "uploading"
          ? "cursor-not-allowed border-zinc-600 bg-zinc-800/50"
          : isDragActive
            ? "cursor-pointer border-blue-500 bg-blue-500/10"
            : "cursor-pointer border-zinc-600 bg-zinc-800/30 hover:border-zinc-500 hover:bg-zinc-800/50"
      }`}
    >
      <input {...getInputProps()} />
      ...
      {error && (
        <div className="mt-4 flex items-start justify-center gap-2 text-left text-xs text-red-400">
